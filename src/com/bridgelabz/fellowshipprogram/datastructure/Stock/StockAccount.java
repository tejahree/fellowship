...
public class StockAccount {
	...
	case 1:	System.out.println("enter CustomerID");
			...
			pricePerShare=scanner.nextInt();
			 customerList.add(new CustomerInfo(customerId, nameOfCustomer, amountOfCustomer, numberOfShare,pricePerShare));
		
		break;
	case 2:System.out.println("enter companyID\n");
			...
			companyList.add(new CompanyShare(companyIdNumber,stockSymbol, numberOfCompanyShare, priceForEachShare, amountOfCompay));
		break;
	case 3:System.out.println("thanku for adding info--\n");
		break;
	}
	...
	switch(option2) {
			case 1:		System.out.println("-----WEL-COME Customer----");
											...
											switch (option3) {
												case 1:System.out.println("----------Buy--------");
														...
														if(custoshare<companyList.get(indexOfCompany).getNumberOfShare()) {
															int amountvalue=custoshare*(companyList.get(indexOfCompany).getPriceForEachShare());
														if(customerList.get(indexOfCustomer).getAmountOfCustomer()>amountvalue) {
														    //this is amount chainging customer
															int newCustAmount=(customerList.get(indexOfCustomer).getAmountOfCustomer())-amountvalue;
															customerList.get(indexOfCustomer).setAmountOfCustomer(newCustAmount);
															//share change customer
															int newCustShare=(customerList.get(indexOfCustomer).getNumberOfShare())+custoshare;
															customerList.get(indexOfCustomer).setNumberOfShare(newCustShare);
															//company amount change
															int newCompAmount=(companyList.get(indexOfCompany).getAmountOfCompay())+amountvalue;
														    companyList.get(indexOfCompany).setAmountOfCompay(amountvalue);
														    //company share change
														    int newCompShare=(companyList.get(indexOfCompany).getNumberOfShare())-custoshare;
														    companyList.get(indexOfCompany).setNumberOfShare(newCompShare);
														    transactid=customerList.get(indexOfCustomer).getNameOfCustomer()+companyList.get(indexOfCompany).getStockSymbol();
														   ...
														   TransacAmount=amountvalue;
														    trasacList.add(new Transaction(transactid, buyerName, sellerName, dateOfCp, TransacAmount));
												...
												case 2: System.out.println("----------sell---------");
														...
														if(Amount<(companyList.get(indexOfCompany).getAmountOfCompay())) {
															int newCustAmount=(customerList.get(indexOfCustomer).getAmountOfCustomer())+Amount;
															customerList.get(indexOfCustomer).setAmountOfCustomer(newCustAmount);
															//customer share change
															int newCustShare=(customerList.get(indexOfCustomer).getNumberOfShare())-SellShare;
															customerList.get(indexOfCustomer).setNumberOfShare(newCustShare);
															//company amount change
															int newCompAmount=(companyList.get(indexOfCompany).getAmountOfCompay())-Amount;
															companyList.get(indexOfCompany).setAmountOfCompay(newCompAmount);
															//company share change
															int newCompShare=(companyList.get(indexOfCompany).getNumberOfShare())+SellShare;
															companyList.get(indexOfCompany).setNumberOfShare(newCompShare); 
															 transactid=customerList.get(indexOfCustomer).getNameOfCustomer()+companyList.get(indexOfCompany).getStockSymbol();
												...
												break;
												default:System.out.println("Enter valide option\n");
												break;
											}
										}
					...
					break;
			case 2:			System.out.println("-----WEL-COME Company----");
									...
									for(int j=0;j<customerList.size();j++) {
										if(cusId==customerList.get(j).getCustomerId()) {
											...
											if(compShare<customerList.get(indexOfCustomer).getNumberOfShare()) {
												int amountvalue=compShare*(customerList.get(indexOfCustomer).getPricePerShare());
											if(amountvalue<companyList.get(indexOfCompany).getAmountOfCompay()) {
											    //this is amount chainging company
												int newCompAmount=(companyList.get(indexOfCompany).getAmountOfCompay())-amountvalue;
												companyList.get(indexOfCompany).setAmountOfCompay(newCompAmount);
												//share change company
												int newCompShare=(companyList.get(indexOfCompany).getNumberOfShare())+compShare;
												companyList.get(indexOfCompany).setNumberOfShare(newCompShare);
												//customer amount change
												int newCustAmount=(customerList.get(indexOfCustomer).getAmountOfCustomer())+amountvalue;
												customerList.get(indexOfCustomer).setAmountOfCustomer(newCustAmount);
											    //customer share change
											    int newCustShare=(customerList.get(indexOfCustomer).getNumberOfShare())-compShare;
											    customerList.get(indexOfCustomer).setNumberOfShare(newCustShare);
											    transactid=companyList.get(indexOfCompany).getStockSymbol()+customerList.get(indexOfCustomer).getNameOfCustomer();
											    buyerName=companyList.get(indexOfCompany).getStockSymbol();
											    sellerName=customerList.get(indexOfCustomer).getNameOfCustomer();
											    dateOfCp=df.format(dateobj);
											   TransacAmount=amountvalue;
		...
		break;
			case 3: System.out.println("thankyou for operation\n");
			break;
		default: System.out.println("Enter valide option\n");
		break;
	}
	...
	listOfCustomer.setCustomerList(customerList);
	listOfCompany.setListOfCompanyShare(companyList);
	listOfTransactionList.setListOfTransaction(trasacList);
